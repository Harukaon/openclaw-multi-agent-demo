...
3. Platform 按 `group_id` 做成员校验、群内广播、稳定序号和 ACK。
4. OpenClaw Channel Plugin 将每个 `group_id` 映射为独立的 OpenClaw 群聊 Session。
...
- Platform REST 写操作可由 `ADMIN_TOKEN` 保护；单机验收仍只监听宿主机 loopback，正式公网反代前必须启用该 token 与 HTTPS。
...
- `message { group, seq, sender, content, mentions, deliveryContext }`
- `replay.end { groupId, lastSeq }`
- `error { code, message }`
- `pong`

`deliveryContext` 至少包含：`groupId`、`groupName`、`mentionState`、`selfMessage`。Mention 状态限定在当前 Group：`SELF`、`DIRECT`、`OTHER`、`NONE`。
...
- 把 Platform 的 sender、mentions、delivery context 注入当前 Agent turn；
- 将 OpenClaw 的文本回复通过同一连接写回原 `group_id`；
- ACK、断线重连和每群 replay。
...
6. 三个真实 OpenClaw + 两个/三个 Group，确认只向成员 Agent 广播。
